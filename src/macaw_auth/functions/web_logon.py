...
def main(args):
    credentials = Configuration(
        "credential", args["PROFILE"], args["credential_file"]
    )
    url_credentials = {}
    url_credentials["sessionId"] = credentials.get_config_setting(
        "aws_access_key_id"
    )
    url_credentials["sessionKey"] = credentials.get_config_setting(
        "aws_secret_access_key"
    )
    url_credentials["sessionToken"] = credentials.get_config_setting(
        "aws_session_token"
    )
    ...
    # Including session_duration causes a failure when making a request
    # using credentials
    request_parameters_no_duration = request_parameters
    request_parameters += "&SessionDuration={}".format(
    ...
    # If request fails, retry without session duration
        ...
        request_url = (
            ...
            + request_parameters_no_duration
        )
        r = requests.get(request_url, verify=args["no_ssl"])
        signin_token = json.loads(r.text)
    ...
    # If a user is already logged in to the console, opening the browser with
    # new credentials will fail. Log out first, then open another window 3
    # secounds later
    ...
    webbrowser.open_new(
        f"https://{region}.console.aws.amazon.com/console/logout!doLogout"
    )
    sleep(3)
