...
def main(args) -> None:
    ...
    config = Configuration(
        args["PROFILE_NAME"], args["config_file"], **client_parameters
    )
    ...
    user_creds = UserCredentials(
        validation.username,
        config.get_config_setting("identity_url"),
        args["auth_type"],
        args["no_ssl"],
        args["reset_password"],
        config.get_config_setting("enable_keyring"),
    )
    sts_service = AWSSTSService(
        config.get_config_setting("region"), verify_ssl=args["no_ssl"]
    )
    sts_service.login(
        config.get_config_setting("account_number"),
        config.get_config_setting("idp_name"),
        config.get_config_setting("role_name"),
        user_creds.assertion,
        args["target_profile"],
        args["credential_file"],
        config.get_config_setting("partition"),
        config.get_config_setting("path"),
        int(config.get_config_setting("session_duration")),
