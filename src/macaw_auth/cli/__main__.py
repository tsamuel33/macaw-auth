...
def main() -> None:
    ...
    client_config = Configuration('user', arg_to_string(args['SOURCE_PROFILE']),
                                #TODO - Add functionality for config file location
                                # args['config_file'],
    ...
    roles = AWSSTSService(user_creds.assertion, client['principal_arn'], client['role_arn'], int(client['session_duration']), client['region'])
    aws_creds = AWSCredentials('credential', arg_to_string(args['target_profile']),
                               #TODO - Add functionality for credential file location
